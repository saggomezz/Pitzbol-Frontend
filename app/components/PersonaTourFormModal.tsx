...
const DURACIONES = [
  "2 horas", "3 horas", "4 horas", "Medio dÃ­a (5-6 h)",
  "DÃ­a completo (8+ h)", "2 dÃ­as", "3 dÃ­as o mÃ¡s",
];


const QUE_INCLUYE = [
  "Agua", "Transporte", "Comida", "Entradas al lugar",
  "Seguro", "FotografÃ­a", "DegustaciÃ³n", "Alojamiento",
];

const DIAS_SEMANA = ["Lun", "Mar", "MiÃ©", "Jue", "Vie", "SÃ¡b", "Dom"];

const VEHICLE_TYPES = ["Auto / SUV", "Van", "MinibÃºs", "CamiÃ³n", "Otro"];
const MAX_FOTOS = 3;
...
export default function PersonaTourFormModal({ guiaId, guiaNombre, guiaIdiomas = [], onClose, onSuccess }: Props) {
  ...
  const validateStep = (s: number) => {
    ...
    if (s === 0) {
      ...
      if (!form.destino.trim()) e.destino = "Escribe la direcciÃ³n o zona principal";
      if (!form.seVeranAhi && !form.puntoEncuentro.trim()) e.puntoEncuentro = "Indica dÃ³nde se encontrarÃ¡n";
      if (!form.duracion) e.duracion = "Selecciona la duraciÃ³n aproximada";
    }
    if (s === 1) {
      if (!form.descripcion.trim()) e.descripcion = "Agrega una descripciÃ³n del tour";
      ...
      if (form.diasDisponibles.length === 0) e.diasDisponibles = "Selecciona al menos un dÃ­a disponible";
      if (form.incluyeTransporte && form.tipoVehiculo.length === 0) e.tipoVehiculo = "Selecciona el tipo de vehÃ­culo";
  ...
  const handleSubmit = async (ev: React.FormEvent) => {
    ...
    try {
      ...
      const disponibilidad = form.diasDisponibles.length > 0
        ? `${form.diasDisponibles.join(", ")} Â· ${form.horaInicio} â€“ ${form.horaFin}`
        : "";
          ...
          <div className="absolute top-5 left-6 right-6 flex justify-between items-center">
            {step > 0
              ? <button type="button" onClick={() => setStep(s => s - 1)} className="text-[#1A4D2E] hover:text-[#0D601E] flex items-center gap-1 transition-all">
                  <FiChevronLeft size={26} /> <span className="hidden md:block font-bold text-sm italic">AtrÃ¡s</span>
                </button>
              : <div />
            }
            ...
            <motion.div key={step} initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -18 }} transition={{ duration: 0.2 }} className="w-full space-y-5">
              ...
              {step === 0 && (<>
                ...
                <div>
                  ...
                  <input className={inputClass + (errors.titulo ? " border-red-400" : "")} placeholder="Ej: Recorrido a pie por el Centro HistÃ³rico" value={form.titulo} onChange={e => set("titulo", e.target.value)} />
                ...
                <div>
                  ...
                  <div className="relative">
                    <input className={inputClass + " pr-10" + (errors.destino ? " border-red-400" : "")} placeholder="Escribe la calle, colonia o zona del tourâ€¦" value={form.destino} onChange={e => handleDestinoInput(e.target.value)} />
                ...
                <div className="rounded-2xl border border-[#D9E5DB] bg-white p-4">
                  <button type="button" onClick={() => set("seVeranAhi", !form.seVeranAhi)} className="flex w-full items-center justify-between gap-4">
                    <div>
                      <p className="text-sm font-bold text-[#1A4D2E] text-left">Â¿Se verÃ¡n ahÃ­?</p>
                      <p className="text-[10px] text-gray-500 mt-0.5">Â¿El punto de encuentro es el mismo destino?</p>
                    </div>
                  ...
                  <AnimatePresence>
                    {!form.seVeranAhi && (
                      <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="mt-3 overflow-hidden">
                        <label className={labelClass}>DirecciÃ³n de encuentro <span className="text-red-500">*</span></label>
                        <input className={inputClass + (errors.puntoEncuentro ? " border-red-400" : "")} placeholder="Ej: Plaza de la LiberaciÃ³n, frente a la Catedral" value={form.puntoEncuentro} onChange={e => set("puntoEncuentro", e.target.value)} />
                ...
                <div>
                  <label className={labelClass}>DuraciÃ³n estimada <span className="text-red-500">*</span></label>
                  <div className="relative">
              ...
              {/* â”€â”€ PASO 2: Detalles â”€â”€ */}
              {step === 1 && (<>
                <div>
                  <label className={labelClass}>DescripciÃ³n del tour <span className="text-red-500">*</span></label>
                  <textarea className={inputClass + " min-h-28 resize-y" + (errors.descripcion ? " border-red-400" : "")} rows={4} placeholder="Describe el recorrido, las paradas y por quÃ© tu experiencia es distinta." value={form.descripcion} onChange={e => set("descripcion", e.target.value)} />
                ...
                <div>
                  ...
                  {guiaIdiomas.length === 0
                    ? <p className="text-xs text-amber-600 bg-amber-50 border border-amber-200 rounded-xl px-4 py-3">Ve a <strong>Mi Perfil â†’ Idiomas</strong> y agrÃ©galos antes de publicar un paquete.</p>
                ...
                <div>
                  <label className={labelClass}>Â¿QuÃ© incluye la experiencia?</label>
                  <Chips options={QUE_INCLUYE} selected={form.queIncluye} toggle={v => toggleArr("queIncluye", v)} />
                </div>

                <div>
                  <label className={labelClass}>DÃ­as disponibles <span className="text-red-500">*</span></label>
                  ...
                  <div className="flex items-center gap-3 mt-2">
                    ...
                    <span className="text-[#81C784] font-bold mt-4">â€“</span>
                  ...
                  {form.diasDisponibles.length > 0 && <p className="text-[10px] text-[#6C8870] mt-1">{form.diasDisponibles.join(", ")} Â· {form.horaInicio} â€“ {form.horaFin}</p>}
                </div>
                ...
                <div className="rounded-2xl border border-[#D9E5DB] bg-white p-4">
                  <button type="button" onClick={() => { const next = !form.incluyeTransporte; set("incluyeTransporte", next); if (!next) { set("capacidad", ""); set("tipoVehiculo", []); } }} className="flex w-full items-start justify-between gap-4">
                    <div>
                      <p className="text-sm font-bold text-[#1A4D2E] text-left">Â¿Incluyes transporte?</p>
                      <p className="text-[10px] text-gray-500 mt-0.5">ActÃ­valo si trasladas al grupo dentro de la experiencia.</p>
                    </div>
                  ...
                  <AnimatePresence>
                    {form.incluyeTransporte && (
                      <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="mt-4 space-y-4 overflow-hidden">
                        <div><label className={labelClass}>Tipo de vehÃ­culo <span className="text-red-500">*</span></label><Chips options={VEHICLE_TYPES} selected={form.tipoVehiculo} toggle={v => toggleArr("tipoVehiculo", v)} />{errors.tipoVehiculo && <p className={errClass}>{errors.tipoVehiculo}</p>}</div>
              ...
              {/* â”€â”€ PASO 3: Fotos â”€â”€ */}
              {step === 2 && (<>
                {submitError && <div className="bg-red-50 border border-red-200 text-red-700 text-xs px-4 py-3 rounded-xl">{submitError}</div>}
                <p className="text-sm text-[#6C8870] text-center">La primera imagen serÃ¡ la portada del paquete.</p>
                ...
                <div className="bg-[#F7FBF7] rounded-2xl p-4 text-xs text-[#5E7862] space-y-1.5 border border-[#E3ECE4]">
                  <p>âœ“ Usa una portada limpia y fotos reales del recorrido</p>
                  <p>âœ“ Describe claramente la experiencia y lo que incluye</p>
                  <p>âœ“ Indica disponibilidad para evitar dudas al turista</p>
                </div>
              </>)}
