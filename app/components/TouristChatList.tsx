...
export default function TouristChatList({ touristId, onSelectChat }: TouristChatListProps) {
  ...
  const fetchChats = useCallback(async () => {
    try {
      ...
      const url = `${BACKEND_URL}/api/chat/user/${touristId}?userType=tourist&_t=${timestamp}`;
  ...
  if (error) {
    return (
      <div className="bg-red-50 border border-red-200 rounded-lg p-6 text-center">
        ...
        <button
          onClick={() => fetchChats()}
          className="bg-[#1A4D2E] hover:bg-[#143d24] text-white px-6 py-2 rounded-lg transition-colors font-medium"
        >
          Reintentar
        </button>
      </div>
            ...
            <div className="relative w-12 h-12 shrink-0">
              {chat.guidePhoto ? (
                ...
                <div className="bg-[#1A4D2E] text-white w-12 h-12 rounded-full flex items-center justify-center text-lg font-bold">
                  {chat.guideName.charAt(0).toUpperCase()}
                </div>
              )}
            </div>
